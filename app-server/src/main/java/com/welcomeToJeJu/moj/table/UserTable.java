...
public class UserTable extends JsonDataTable<User> implements DataProcessor{
  ...
  private void likedUserDelete(Request request, Response response) {
    ...
    User loginUser = findByName(loginUserName);
    User likedUser = findByName(likedUserName);
    if(likedUser != null) {
      likedUser.getLikedUsers().remove(loginUser.getNickName());
  ...
  private void likedUserList(Request request, Response response) {
    ...
    User loginUser =findByName(user.getNickName());
    List<String> likedUserList = new ArrayList<>();
    for(User u : list) {
      for (String s : u.getLikedUsers()) {
        if(s.equals(loginUser.getNickName())) {
          likedUserList.add(u.getNickName());
        }
      }
    }
    response.setStatus(Response.SUCCESS);
  ...
  private void likedUserInsert(Request request, Response response) {
    ...
    User likedUser = findByName(likedUserName);
    if(likedUser != null) {
      likedUser.getLikedUsers().add(loginUser.getNickName());
      response.setStatus(Response.SUCCESS);
    } else {
      response.setStatus(Response.FAIL);
      response.setValue("등록된 유저 없음!");
